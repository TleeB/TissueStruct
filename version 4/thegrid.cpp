...
int main(){//Forgot the double parenthesis
  ...
  static RNGType rng(seed);
  cout <<"Seed: " << seed << endl ;
  //static RNGType rng(static_cast<unsigned int>(2)); //In order to produce reproducible results set a defined seed
  ...
  double alpha = 2;//Ratio of fibroblasts to myocytes in final distribution
      ...
      if (grid_toggle == true){
        if ( (r-1) < 0){
          for (int i = 0; i < cell_length; i++){
            if ((c+i) < NCOLS){
              neighbor.push_back(grid[r+1][c+i]);//changed r to r+1, If at the bottom, write bottom neighbor twice
            }
          }
        }
        else{
        ...
        if ((r+1) > NROWS-1){
          for (int i = 0; i < cell_length; i++){
            if ((c+i) < NCOLS){
              neighbor.push_back(grid[r-1][c+i]);//changed r to r-1, If at the top, write above row twice
            }
          }
        }
        else{
