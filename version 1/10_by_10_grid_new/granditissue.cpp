...
void PDE(vector<vector<double> > &, vector<vector<double> > &, vector <vector<int> >);
...
int main(){
  ...
  while ( num < num_stim )	{
    ...
    while ( count < bcl_int ){
      ...
      for (int x = 0; x<CELLS_myo; x++){
        ...
        /* Current injection */
        if ( (( count <= stim_dur_int) && (stim_myo[0][x] == 1))&& (num > stim_equil) ){
          I_stim = stim_mag;
      ...
      for (int x = 0; x<CELLS_fib; x++){
        ...
        // Current injection
        if ( (( count <= stim_dur_int)&& (stim_fib[0][x] == 1))&& (num> stim_equil)) {
          I_stim_fib = -stim_mag*10e6;
        }
        else {
          I_stim_fib = 0.0;
      ...
      PDE(y, yf, neighbors_myo);
      //cout << "I out function: " << ydot[0][0] << endl;
      //cout<<"first pde"<<endl;
      PDE(y, yf, neighbors_fib);

      for (int x = 0; x<CELLS_myo; x++){
        if(count%100==0) fprintf(output4,"%.12f\t",y[x][0]);
      }
      if(count%100==0) fprintf(output4,"\n");
  ...
  void PDE( vector< vector<double> >  &Vmyo, vector< vector<double> > &Vfib, vector< vector<int> >  neighbor ){//& pass by reference changes the original parameter
    double G, Vj;
        ...
        if(neighbor[p][0] > 0){//If self is a myocyte, determine neighbor
          if(neighbor[p][N] > 0){ //If neighbor is a myocyte
            if((neighbor[p][N] == p+2)||(neighbor[p][N] == p-0)){//lazy way of indicating end to end connections
              ...
              G = .02;  //If the neighbor is end to end then conductance should be 600
            }
