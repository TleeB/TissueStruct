...
int num_stim=5;//must be greater than 1 since first stim cycle is for equilib
double stim_mag=25;
double stim_dur=3;
double bcl=1000;
double DT = 0.005;
...
int main(){
  ...
  while ( num < num_stim )	{
    ...
    while ( count < bcl_int ){
      ...
      for (int x = 0; x<CELLS_myo; x++){
        ...
        /* Current injection */
        if ((( count <= stim_dur_int) && (stim_myo[0][x] == 1)) ) {//no stimulation for one cycle to reach equilib.
          I_stim = stim_mag;
        }
        else {
          I_stim = 0.0;
  ...
  PDE(temp, neighbors_myo);
  PDE(temp2, neighbors_fib);
    ...
    for (int p = 0; p < neighbor.size(); p++){
      for (int N = 1; N < neighbor[p].size(); N++){//starts at 1 because first entry (0) is self
        if(neighbor[p][N] > 0){ //If neighbor is a myocyte
          /*if((neighbor[p][N] == p+1)||(neighbor[p][N]== p-1)){//lazy way of indicating end to end connections
            G = 600.;
          }
          else{
