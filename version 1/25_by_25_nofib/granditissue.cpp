...
char output_file_name3[30] = "calcium.dat";
...
template <size_t size_y> //Use templates to allow for arrays of different dimensions
void PDE( double V[][size_y], double Itotal[][size_y], vector<vector<int> > & neighbor, bool fibflag ){
  double G;
  int index;
  for (int p = 0; p < neighbor.size(); p++){
    //cout <<"Cell number: "<<p<<endl;
    for (int N = 1; N < neighbor[p].size(); N++){//starts at 1 because first entry (0) is self
      //cout << "Neighbor: "<< neighbor[p][N] <<endl;
      if (fibflag){
        G = 3.;
        index = 0;
      ...
      if(neighbor[p][N] > 0){ //If neighbor is a myocyte
        index = 39;
        if((neighbor[p][N] == p+2)||(neighbor[p][N]== p-0)){//lazy way of indicating end to end connections, because array index is one less than actual cell number, must and 2 and subtract zeo
          G = 600.;
         // cout << "End-to-end connection"<<endl;
        }
        else{
      ...
      Itotal[p][index] += G * DT * (V[p][index] - V[ abs(neighbor[p][N]) ][index] );
...
//void PDE(double V[][], vector <vector<int> >  &);
...
int main(){
  ...
  cout<< "Before loading vectors" << endl;
  ...
  while ( num < num_stim )	{
    ...
    while ( count < bcl_int ){
      ...
      for (int x = 0; x<CELLS_myo; x++){
        ...
        //I_Na: Fast Na Current
        mss = 1 / pow((1 + exp( -(56.86 + y[x][39]) / 9.03 )),2);
        taum = 0.1292 * exp(-pow(((y[x][39]+45.79)/15.54),2)) + 0.06487 * exp(-pow(((y[x][39]-4.823)/51.12),2));

        if (y[x][39] >= -40.) {
          ah = 0;
          bh = (0.77 / (0.13*(1 + exp( -(y[x][39] + 10.66) / 11.1 ))));
          aj = 0;
          bj = ((0.6 * exp( 0.057 * y[x][39])) / (1 + exp( -0.1 * (y[x][39] + 32) )));
        }
        else{
          ah = (0.057 * exp( -(y[x][39] + 80) / 6.8 ));
          bh = ((2.7 * exp( 0.079 * y[x][39]) + 3.1e5 * exp(0.3485 * y[x][39])));
          aj = (((-2.5428 * 1.0e4*exp(0.2444*y[x][39]) - 6.948e-6 * exp(-0.04391*y[x][39])) * (y[x][39] + 37.78)) / (1 + exp( 0.311 * (y[x][39] + 79.23) )));
          bj = ((0.02424 * exp( -0.01052 * y[x][39] )) / (1 + exp( -0.1378 * (y[x][39] + 40.14) )));
        }
        tauh = 1 / (ah + bh);
        hss = 1 / pow((1 + exp( (y[x][39] + 71.55)/7.43 )),2);
        tauj = 1 / (aj + bj);
        jss = 1 / ((1 + exp( (y[x][39] + 71.55)/7.43 ))*(1 + exp( (y[x][39] + 71.55)/7.43 )));

        ydot[x][1] = (mss - y[x][1]) / taum;
        ydot[x][2] = (hss - y[x][2]) / tauh;
        ydot[x][3] = (jss - y[x][3]) / tauj;

        I_Na_junc = Fjunc*GNa*y[x][1]*y[x][1]*y[x][1]*y[x][2]*y[x][3]*(y[x][39]-ena_junc);
        I_Na_sl = Fsl*GNa*y[x][1]*y[x][1]*y[x][1]*y[x][2]*y[x][3]*(y[x][39]-ena_sl);
        I_Na = I_Na_junc+I_Na_sl;

        // Late I_na
        aml=0.32*(y[x][39]+47.13)/(1-exp(-0.1*(y[x][39]+47.13)));
        bml=0.08*exp(-y[x][39]/11.0);
        mlss=aml/(aml + bml);
        tauml = 1 / (aml + bml);

        hlss=1/(1+exp((y[x][39]+91)/6.1));
        tauhl=scalehL*233;// modified in HF

        ydot[x][40] = (mlss - y[x][40]) / tauml;//<------------------------??
        ydot[x][41] = (hlss - y[x][41]) / tauhl;

        I_NaL_junc = scaleNaL*Fjunc*GNaL*pow(y[x][40],3)*y[x][41]*(y[x][39]-ena_junc);//modified in HF
        I_NaL_sl =scaleNaL*Fsl*GNaL*pow(y[x][40],3)*y[x][41]*(y[x][39]-ena_sl); //modified in HF
        I_NaL = I_NaL_junc+I_NaL_sl;

        //I_nabk: Na Background Current
        I_nabk_junc = scaleNab*Fjunc*GNaB*(y[x][39]-ena_junc); //modified in HF
        I_nabk_sl = scaleNab*Fsl*GNaB*(y[x][39]-ena_sl);//modified in HF
        I_nabk = I_nabk_junc+I_nabk_sl;

        //I_nak: Na/K Pump Current
        sigma = (exp(Nao/67.3)-1)/7;
        fnak = 1/(1+0.1245*exp(-0.1*y[x][39]*FoRT)+0.0365*sigma*exp(-y[x][39]*FoRT));
        ...
        xrss = 1/(1+exp(-(y[x][39]+10)/5));
        tauxr = 550/(1+exp((-22-y[x][39])/9))*6/(1+exp((y[x][39]-(-11))/9))+230/(1+exp((y[x][39]-(-40))/20));
        ydot[x][12] = (xrss-y[x][12])/tauxr;
        rkr = 1/(1+exp((y[x][39]+74)/24));

        //Omitted commented lines
        I_kr =gkr*y[x][12]*rkr*(y[x][39]-ek);
        //Omitted commented lines
        //Using none markov IKS
        gks_junc=1*0.0035;
        gks_sl=1*0.0035; //FRA
        xsss = 1 / (1+exp(-(y[x][39] + 3.8)/14.25)); // fitting Fra
        tauxs=990.1/(1+exp(-(y[x][39]+2.436)/14.12));
        ydot[x][13] = (xsss-y[x][13])/tauxs;
        I_ks_junc = Fjunc*gks_junc*y[x][13]*y[x][13]*(y[x][39]-eks);
        I_ks_sl = Fsl*gks_sl*y[x][13]*y[x][13]*(y[x][39]-eks);
        I_ks = I_ks_junc+I_ks_sl;

        //I_kp: Plateau K current
        kp_kp = 1/(1+exp(7.488-y[x][39]/5.98));
        I_kp_junc = Fjunc*gkp*kp_kp*(y[x][39]-ek);
        I_kp_sl = Fsl*gkp*kp_kp*(y[x][39]-ek);
        ...
        xtoss = 1/(1+exp(-(y[x][39]-19.0)/13));
        ytoss = 1/(1+exp((y[x][39]+19.5)/5));
        tauxtos = 9/(1+exp((y[x][39]+3.0)/15))+0.5;
        tauytos = 800/(1+exp((y[x][39]+60.0)/10))+30;
        ydot[x][8] = (xtoss-y[x][8])/tauxtos;
        ydot[x][9] = (ytoss-y[x][9])/tauytos;

        I_tos = scaleto*GtoSlow*y[x][8]*y[x][9]*(y[x][39]-ek);    // [uA/uF]   modified in HF

        tauxtof = 8.5*exp(-pow(((y[x][39]+45)/50),2))+0.5;
        tauytof = 85*exp((-pow((y[x][39]+40),2)/220))+7;
        ydot[x][10] = (xtoss-y[x][10])/tauxtof;
        ydot[x][11] = (ytoss-y[x][11])/tauytof;

        I_tof = scaleto*GtoFast*y[x][10]*y[x][11]*(y[x][39]-ek);
        I_to = I_tos + I_tof;

        //I_ki: Time-Independent K Current
        aki = 1.02/(1+exp(0.2385*(y[x][39]-ek-59.215)));
        bki =(0.49124*exp(0.08032*(y[x][39]+5.476-ek)) + exp(0.06175*(y[x][39]-ek-594.31))) /(1 + exp(-0.5143*(y[x][39]-ek+4.753)));
        kiss = aki/(aki+bki);
        I_ki =scaleK1*1* 0.35*sqrt(Ko/5.4)*kiss*(y[x][39]-ek); // modified in HF

        //I_ClCa: Ca-activated Cl Current, I_Clbk: background Cl Current
        I_ClCa_junc = Fjunc*GClCa/(1+KdClCa/y[x][36])*(y[x][39]-ecl);
        I_ClCa_sl = Fsl*GClCa/(1+KdClCa/y[x][37])*(y[x][39]-ecl);
        I_ClCa = I_ClCa_junc+I_ClCa_sl;
        I_Clbk = GClB*(y[x][39]-ecl);

        //Omitted commented lines
        fss = 1/(1+exp((y[x][39]+35)/9))+0.6/(1+exp((50-y[x][39])/20));
        //Omitted commented lines

        dss = 1/(1+exp(-(y[x][39]+5)/6.0));
        taud = dss*(1-exp(-(y[x][39]+5)/6.0))/(0.035*(y[x][39]+5));
        tauf = 1/(0.0197*exp( -pow((0.0337*(y[x][39]+14.5)),2) )+0.02);
        ...
        ibarca_j = pCa*4*(y[x][39]*Frdy*FoRT) * (0.341*y[x][36]*exp(2*y[x][39]*FoRT)-0.341*Cao) /(exp(2*y[x][39]*FoRT)-1);
        ibarca_sl = pCa*4*(y[x][39]*Frdy*FoRT) * (0.341*y[x][37]*exp(2*y[x][39]*FoRT)-0.341*Cao) /(exp(2*y[x][39]*FoRT)-1);
        ibark = pK*(y[x][39]*Frdy*FoRT)*(0.75*y[x][35]*exp(y[x][39]*FoRT)-0.75*Ko) /(exp(y[x][39]*FoRT)-1);
        ibarna_j = pNa*(y[x][39]*Frdy*FoRT) *(0.75*y[x][32]*exp(y[x][39]*FoRT)-0.75*Nao)  /(exp(y[x][39]*FoRT)-1);
        ibarna_sl = pNa*(y[x][39]*Frdy*FoRT) *(0.75*y[x][33]*exp(y[x][39]*FoRT)-0.75*Nao)  /(exp(y[x][39]*FoRT)-1);
        ...
        s1_junc = exp(nu*y[x][39]*FoRT)*y[x][32]*y[x][32]*y[x][32]*Cao;
        s1_sl = exp(nu*y[x][39]*FoRT)*y[x][33]*y[x][33]*y[x][33]*Cao;
        s2_junc = exp((nu-1)*y[x][39]*FoRT)*Nao*Nao*Nao*y[x][36];
        ...
        s2_sl = exp((nu-1)*y[x][39]*FoRT)*Nao*Nao*Nao*y[x][37];
        ...
        //Omitted commented lines
        I_ncx_junc = scaleNCX*Fjunc*IbarNCX*pow(Q10NCX,Qpow)*Ka_junc*(s1_junc-s2_junc)/s3_junc/(1+ksat*exp((nu-1)*y[x][39]*FoRT));//modified in HF
        I_ncx_sl = scaleNCX*Fsl*IbarNCX*pow(Q10NCX,Qpow)*Ka_sl*(s1_sl-s2_sl)/s3_sl/(1+ksat*exp((nu-1)*y[x][39]*FoRT)); //modified in HF
        ...
        I_cabk_junc =scaleCab*Fjunc*GCaB*(y[x][39]-eca_junc);//modified in HF
        I_cabk_sl = scaleCab*Fsl*GCaB*(y[x][39]-eca_sl);//modified in HF
        ...
        /* Current injection */
        if (( count <= stim_dur_int) && (stim_myo[0][x] == 1)) {
          I_stim = stim_mag;
        }
        else {
          I_stim = 0.0;
        ...
        I_tot = I_Na_tot+I_Cl_tot+I_Ca_tot+I_K_tot + I_stim;
        ydot[x][39] = -(I_tot);
        ydot[x][0] = ydot[x][39]; //duplicate membrane potential

        if(count%100==0) fprintf(output,"%.12f\t",y[x][39]);
      ...
      for (int x = 0; x<CELLS_fib; x++){
        ...
        /* Current injection */
        if (( count <= stim_dur_int)&& (stim_fib[0][x] == 1)) {
          I_stim_fib = stim_mag;
        }
        else {
          I_stim_fib = 0.0;
      ...
      //
     // cout<<"myo before: " << ydot[1][39] << endl;

      //double temp[CELLS_myo], temp2[CELLS_myo];

      // set the contents of buf equal to the contents of the first row of multiarray.
      //  memcpy(temp, y[39], CELLS_myo * sizeof(double)); 
      //  memcpy(temp2, yf[0],CELLS_fib * sizeof(double)); 
      //   cout<<" temp before: " << temp[10] << endl;

      //Solve PDE for all neighbors
      PDE(y,ydot, neighbors_myo, 0);
      PDE(yf,yfdot, neighbors_fib, 1);

      //  cout <<"temp after: " << temp[10] << endl;
     // cout <<"myo after: " << ydot[1][39] << endl<<endl;
